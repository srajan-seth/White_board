...
const handleDeleteCanvas = async (id) => {
 ...
 try{ console.log(id,token);

  await toast.promise(
    axios.delete(`http://localhost:5000/api/canvas/delete/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    }),
    {
      loading: "Deleting canvas...",
      success: "Canvas deleted successfully!",
      error: (err) =>
        err?.response?.data?.error || "Failed to delete canvas",
    }
  );
